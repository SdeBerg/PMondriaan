...
/**
 * Creates a hypergraph from a graph in mtx format. Returns a vector with
 * vertices. It is possible to set a start and end vertex number that should
...
pmondriaan::hypergraph read_hypergraph(std::string filename, bulk::world& world) {
	...
	auto result = pmondriaan::hypergraph(vertices, nets);
	
	return result;
}

/**
 * Creates the local hypergraph in parallel from a graph in mtx format. Returns a vector with
 * vertices. 
 
std::vector<pmondriaan::vertex> p_block_read_hypergraph(std::string filename, bulk::world& world) {
	uint64_t s = world.rank();
    uint64_t p = world.active_processors();
	...
	// We have an MxN matrix with L nonzeros
	uint64_t E, V, L;
    ...
    return read_hypergraph(filename, b * s, b * (s + 1) );
} */
