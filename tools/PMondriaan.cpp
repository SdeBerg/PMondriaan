...
int main(int argc, char **argv) {
  ...
  cli_settings settings;
	...
	CLI::Option *fopt = app.add_option("-f, --file", settings.matrix_file, "File including the hypergraph to be partitioned in matrixmarket format");
	fopt->check(CLI::ExistingFile);
	
	app.add_option("-p, --processors", settings.p, "Number of processors to be used", settings.p);
	app.add_option("-k, --parts", settings.k, "Number of parts to partition H into", settings.k);
	
	app.add_option("--eps, --epsilon", settings.eps, "Maximum imbalance of the final partitioning", settings.eps);
	app.add_option("--eta", settings.eta, "Maximum imbalance during the parallel computation", settings.eta);
	
	CLI::Option *wopt = app.add_option("--weights", settings.hypergraph_weights, "How the weights of the vertices should be computed");
	CLI::Option *bopt = app.add_option("--bisect", settings.bisection_mode, "The bisection mode used");
	CLI::Option *copt = app.add_option("--sampling", settings.sampling_mode, "Sampling mode to be used");
    ...
    if (s == 0) {
      ...
      out.close();
		...
		auto H = pmondriaan::read_hypergraph(settings.matrix_file, world, settings.hypergraph_weights);

		recursive_bisect(world, H, settings.bisection_mode, settings.sampling_mode, settings.metric, settings.k, settings.eps, settings.eta, options);
