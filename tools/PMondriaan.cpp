...
int main(int argc, char **argv) {
	...
	int p, k;
	double eps, eta;
	std::string matrix_file, hypergraph_weigths, bisection_mode, metric;
	...
	env.spawn(p, [](bulk::world& world) {
		...
		auto q_settings = bulk::queue<int, double, double, std::string, std::string, std::string, std::string, pmondriaan::options>(world);
		...
		if (s == 0) {
			...
			for (int t = 0; t < p; t++) {
				q_settings(t).send(k, eps, eta, matrix_file, hypergraph_weigths, bisection_mode, metric, options);
			}
		...
		for (const auto& [k_, eps_, eta_, matrix_file_, hypergraph_weigths_, bisection_mode_, metric_, options_] : q_settings) {
            k = k_;
			eps = eps_;
			eta = eta_;
