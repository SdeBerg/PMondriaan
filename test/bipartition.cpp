...
int main() {
    ...
    env.spawn(4, [](bulk::world& world) {
        ...
        srand(world.rank() + 1);
        auto hypergraph =
        pmondriaan::read_hypergraph("../test/data/matrices/gemat11/gemat11.mtx", world, "one");
        if (!hypergraph) {
        ...
        recursive_bisect(world, H, "multilevel", "label propagation", "cutnet",
                         2, 0.03, 0.03, opts);
        ...
        while (count < p) {
            if (s == count) {
                for (auto& v : H.vertices()) {
                    world.log("v: %d, part: %d", v.id(), v.part());
                }
            }
            world.sync();
            count++;
